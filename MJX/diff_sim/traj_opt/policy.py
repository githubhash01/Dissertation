...
# TODO - CHANGE
from gradient_config import GradientMethod, GRADIENT_METHOD
...
# -
# Totally Default Auto-Diff Step Function
# -
...
# -------------------------------------------------------------
# Single Iteration Auto-Diff in MuJoCo step function
# -------------------------------------------------------------
def make_step_fn_single(mx, set_control_fn, fd_cache=None):
    ...
    def step_fn_bwd(res, g):
        ...
        # Create a surrogate model with 1 iteration for backward
        mx_approx = mx.replace(opt=mx.opt.replace(iterations=1))

        # Define the surrogate step (1 iteration)
        def surrogate_step(dx, u):
            dx_with_ctrl = set_control_fn(dx, u)
            return mjx.step(mx_approx, dx_with_ctrl)

        # Compute gradients using the surrogate step
        _, vjp_fn = jax.vjp(surrogate_step, dx_in, u_in)
        grad_dx, grad_u = vjp_fn(g)

        if fd_cache:
        # Apply the same "sensitivity_mask" and "inner_idx" as FD for fairness
            dx_array, _ = ravel_pytree(grad_dx)
            dx_array_masked = dx_array.at[fd_cache.inner_idx].multiply(fd_cache.sensitivity_mask)
            grad_dx_masked = fd_cache.unravel_dx(dx_array_masked)

            return grad_dx_masked, grad_u

        #print(f"Backward pass gradient (g): {g.qpos[0]}")  # Example: print first qpos element
        #print("grad_u : ", grad_u)
        #jax.debug.print("Gradient qpos[0]: {g}", g=g.qpos[0])
        has_nan = jnp.any(jnp.isnan(g.qpos)) | jnp.any(jnp.isnan(g.qvel))
        # convert has_nan to boolean
        jax.debug.print("has_nan : {}", has_nan)

        return grad_dx, grad_u
...
# -------------------------------------------------------------
def make_step_fn_finite_difference(
        mx,
...
def simulate_trajectories(
    ...
    def single_trajectory(qpos_init, qvel_init, key):
        ...
        #print("policy.py: single_trajectory")
        ...
        def scan_step_fn(carry, _):
            ...
            dx = step_fn(dx, u)  # step function (probably overwritten)

            c = running_cost_fn(dx)
            state = jnp.concatenate([dx.qpos, dx.qvel, dx.sensordata])
            #print("policy.py: scan_step_fn COMPLETE")
...
def make_loss_multi_init(
    ...
    #print("policy.py: building FD cache")
    ...
    if GRADIENT_METHOD == GradientMethod.AUTO_DIFF:
        print("policy.py: Using Auto-Diff policy for policy optimization")
        #step_fn = make_step_fn(mx, set_control_fn)
        step_fn = make_step_fn_default(mx, set_control_fn)

    if GRADIENT_METHOD == GradientMethod.IMPLICIT_DIFFERENTIATION:
        print("policy.py: Using Implicit Differentiation based custom VJP step function for policy optimization")
        step_fn = make_step_fn_default(mx, set_control_fn)

    if GRADIENT_METHOD == GradientMethod.FINITE_DIFFERENCE:
        print("policy.py: Using FD based custom VJP step function for policy optimization")
        step_fn = make_step_fn_finite_difference(mx, set_control_fn, fd_cache)
    ...
    def multi_init_loss_stoch(params, static, keys):
        ...
        # jax.debug.print()
        jax.debug.print("total_cost : {}", total_cost)
        return total_cost
...
class Policy:
    ...
    def solve(self, model: equinox.Module, optim, state, batch_size, max_iter=100):
        ...
        #print("policy.py: JIT compilation started")
        grad_loss = equinox.filter_jit(jax.jacrev(self.loss))
        #print("policy.py: JIT compilation finished")
        opt_model = None
        key = jax.random.PRNGKey(10)

        #print("policy.py: Starting policy optimization")
        for i in range(max_iter):
            ...
            print("policy.py: Starting Gradient calculation")
            g = grad_loss(params, static,key_batch)


            global TOTAL_BACKPROPS
            print(f"Total Backdrops {TOTAL_BACKPROPS}")
            TOTAL_BACKPROPS = 0


            print("policy.py: Gradient calculated")
            f_val = self.loss(params, static,key_batch)
            print("policy.py: Loss calculated")
            updates, state = optim.update(g, state, model)
            print("policy.py: Updates calculated")
