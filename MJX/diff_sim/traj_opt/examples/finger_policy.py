...
from diff_sim.traj_opt.policy import (
    simulate_trajectories, make_loss_multi_init, make_step_fn_finite_difference, build_fd_cache
)
...
if __name__ == "__main__":
    ...
    """
    Solver Types and their corresponding values:
    
    PGS (Projected Gauss-Seidel) - 0
    CG (Conjugate Gradient) - 1
    Newton - 2
    """
    solver_dict = {0: "PGS", 1: "CG", 2: "Newton"}
    """
    Print the mjx object to see the default solver and iterations
    """
    print(mx.opt)
    # overwrite the default solver and iterations
  #  mx = mx.replace(opt=mx.opt.replace(solver=2, iterations=1))

    print(f"finger_policy.py: Model Loaded\nSolver: {solver_dict[mx.opt.solver]}, Iterations: {mx.opt.iterations}, Tolerance: {mx.opt.tolerance}")
    ...
    def running_cost(dx):
        #print("finger_policy.py: Running cost")
    ...
    def terminal_cost(dx):
        #print("finger_policy.py: Terminal cost")
        pos_finger = dx.qpos[2]
        #print(pos_finger)
        touch = dx.sensordata[0]
        #print(touch)
        ...
        p_target = dx.sensordata[4:7]
        #print(p_target)
        #print(10. * jnp.sum((p_finger - p_target)**2) + 4.* touch * pos_finger**2)
        #print("finger_policy.py: Terminal cost Completed")
    ...
    #print("finger_policy.py: Defining loss function")
    ...
    # JIT the gradient
    #print("JIT compilation started")
    #grad_loss = equinox.filter_jit(jax.jacrev(loss_fn))
    ...
    #print("finger_policy.py: Starting policy optimization")
    ...
    print("finger_policy.py: Evaluating final performance")
    print("finger_policy.py: using FD based custom VJP step function for policy evaluation")
    step_fn = make_step_fn_finite_difference(mx, set_control, fd_cache)
    ...
    print("finger_policy.py: Visualizing the trajectories")
