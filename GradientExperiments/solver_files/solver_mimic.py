...
def backward(residual: Tuple[Model, Data, Data], dx_next_bar: Data) -> Tuple[Model, Data]:
    model, dx, dx_next = residual

    # Extract the solved acceleration (z*) and its cotangent (z*_bar)
    z_star = dx_next.qacc  # z* (the fixed point)
    z_star_bar = dx_next_bar.qacc  # z*_bar

    # Define R_fn that maps a guess for qacc to the residual f.
    def R_fn(q):
        # Replace the solved acceleration with q.
        d_new = dx_next.replace(qacc=q)
        return f(model, d_new)

    # Compute the Jacobian J = dR/dq at q = z_star.
    J = jax.jacobian(R_fn)(z_star)  # Shape: (nv, nv)

    # Build the system matrix: I - J^T.
    I = jnp.eye(J.shape[0])
    A = I - J.T

    # Solve the linear system: A * u = z_star_bar.
    # Note: You might need to handle potential singularity or add damping.
    u = jax.scipy.linalg.solve(A, z_star_bar, assume_a='sym')

    # Now, compute gradients with respect to model and dx.
    def f_for_params(model, d):
        # Fix qacc to be the solved value (z_star).
        return f(model, d.replace(qacc=z_star))

    _, vjp_params = jax.vjp(f_for_params, model, dx)
    grad_model, grad_dx = vjp_params(u)

    return grad_model, grad_dx


solve.defvjp(solve.forward, solve.backward)  # register the forward and backward functions with the custom vjp
