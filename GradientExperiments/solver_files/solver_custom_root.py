...
def solve(m: Model, dx: Data) -> Data:
    """Standard forward solver using solve_iterations."""
    return solve_iterations(m, dx)

def solve_forward(m: Model, dx: Data) -> tuple[Data, tuple[Model, Data]]:
    """Forward pass: compute dx_next and store residuals for backprop."""
    dx_next = solve(m, dx)  # Solves for the next state
    return dx_next, (m, dx)

def solve_backward(residual: Tuple[Model, Data], dx_next_bar: Data) -> Tuple[None, Data]:
    """
    Implicit differentiation of the solver using jax.lax.custom_root.

    Instead of unrolling the solver iterations, we define a root-finding problem.
    """
    model, dx = residual

    # Define residual function f(dx) = 0
    def constraint_residual(dx):
        """Residual function defining constraint violation using dx.qacc."""
        return solve_iterations(model, dx).qacc - dx.qacc  # Enforce convergence

    # Solve function: standard solver
    def solve_fn(f, x0):
        """Solve f(dx) = 0 using the original solver."""
        return solve_iterations(model, x0)  # Use standard MuJoCo solve

    # Tangent solver: Implicit differentiation
    def tangent_solve_fn(g, y):
        """Solve the linearized system g(x) = y using implicit differentiation."""
        return jax.scipy.linalg.solve(jax.jacobian(g)(y), y)

    # Use custom_root to compute implicit differentiation
    dx_bar = jax.lax.custom_root(
        constraint_residual,
        dx_next_bar,
        solve_fn,
        tangent_solve_fn
...
solve.defvjp(solve_forward, solve_backward)  # Register forward and backward pass
